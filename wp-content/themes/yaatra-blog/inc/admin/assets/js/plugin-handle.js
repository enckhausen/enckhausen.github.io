...
jQuery(document).ready(function($) {
    ...
    const setupEventHandlers = (status) => {
        $('.cv-get-started, .cv-activate-demo-import-plugin').on('click', function() {
            const _this = $(this);
            yaatra_blog_do_plugin(`yaatra_blog_${status}_plugin`, _this);
        });
    };
     ...
     */
    const yaatra_blog_do_plugin = function(ajax_action, _this) {
        $.ajax({
            method: "POST",
            url: WpAjaxurl,
            data: {
    ...
    $('.cv-action-btn').on('click', function() {
        ...
        if (actionBtnStatus === 'install') {
            yaatra_blog_do_free_plugin( 'yaatra_blog_install_free_plugin', pluginSlug, _this );
        } else if (actionBtnStatus === 'active') {
            yaatra_blog_do_free_plugin( 'yaatra_blog_activate_free_plugin', pluginSlug, _this );
        }
    });
     ...
     */
    const yaatra_blog_do_free_plugin = function(ajax_action, pluginSlug, _this) {
        $.ajax({
            method: "POST",
            url: WpAjaxurl,
            data: {
